...
surveyTableSummary <- params$surveyTableSummary
...
# Data Structure
surveyTableStructure <- surveyTableSummary$surveyTableStructure
quadratsPerYear <- surveyTableStructure$quadratsPerYear
quadratsPerYearGroup <- surveyTableStructure$quadratsPerYearGroup
...
```{r pseudoMAVISVersion, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

cat("**Version:** ", as.character("v0.95"), sep = "")
...
# Introduction
This report contains user-selected outputs of a pseudoMAVIS application session.
The selected outputs are shown in Table \@ref(tab:selectedOutputs).
...
kableExtra::kbl(x = selectedOutputs, format = "latex", booktabs = TRUE,
                ...
                caption = "The pseudoMAVIS results selected for inclusion in this report.") |>
...
As detailed in the survey data validation section of pseudoMAVIS a sample size 
of at least five quadrats are reccomended per group and per year.
...
However, at present pseudoMAVIS does not prevent analysis if the number
of quadrats per group and year is less than five, leaving the user to interpret
their the results in the context of the submitted data. 
...
if("nvcAssignmentResultsSite" %in% reportOptions){
  ...
  cat("## NVC Assignment, by Site")
  
  nvcAssignmentSite_prepped <- nvcAssignmentSite |>
    dplyr::mutate_if(is.numeric, round, digits = 2)

  kableExtra::kbl(x = nvcAssignmentSite_prepped, format = "latex", booktabs = TRUE,
                  longtable = TRUE, linesep = "",
...
if("nvcAssignmentResultsGroup" %in% reportOptions){
  ...
  cat("## NVC Assignment, by Group")
  
  nvcAssignmentGroup_prepped <- nvcAssignmentGroup |>
    dplyr::mutate_if(is.numeric, round, digits = 2)

  kableExtra::kbl(x = nvcAssignmentGroup_prepped, format = "latex", booktabs = TRUE,
                  longtable = TRUE, linesep = "",
...
if("nvcAssignmentResultsQuadrat" %in% reportOptions){
  ...
  cat("## NVC Assignment, by Quadrat")
  
  nvcAssignmentQuadrat_prepped <- nvcAssignmentQuadrat |>
    dplyr::mutate_if(is.numeric, round, digits = 2)

  kableExtra::kbl(x = nvcAssignmentQuadrat_prepped, format = "latex", booktabs = TRUE,
                  longtable = TRUE, linesep = "",

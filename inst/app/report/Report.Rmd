...
# Retrieve region
region <- params$setupData$region
...
vcAssignment <- params$vcAssignment
...
# VC Assignment
vcAssignmentPlot_Jaccard <- vcAssignment$vcAssignmentPlot_Jaccard
vcAssignmentSite_Czekanowski <- vcAssignment$vcAssignmentSite_Czekanowski
vcAssignmentGroup_Czekanowski <- vcAssignment$vcAssignmentGroup_Czekanowski
vc_comms_10 <- vcAssignment$vc_comms_10
vc_comms_subcomms_10 <- vcAssignment$vc_comms_subcomms_10
...
# Diversity
diversityYear <- diversityAnalysis$diversityYear
diversityGroup <- diversityAnalysis$diversityGroup
diversityQuadrat <- diversityAnalysis$diversityQuadrat

# MVA Local Reference, Restricted
mvaLocalRefRestricted_quadrats_survey <- mvaLocalRefRestrictedResults$dca_results_sample_site
mvaLocalRefRestricted_centroids <- mvaLocalRefRestrictedResults$pquad_centroids
...
mvaLocalRefUnrestricted_centroids <- mvaLocalRefUnrestrictedResults$pquad_centroids
...
mvaNationalRef_centroids <- mvaNationalRefResults$pquad_centroids
...
cat("**Version:** ", as.character("v1.2.3"), sep = "")

```

```{r region, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
region_print <- names(RMAVIS:::region_options[which(RMAVIS:::region_options == region)])

cat("**Region:** ", as.character(region_print), sep = "")
...
available_report_options <- RMAVIS:::reportOptions_options

if(!is.null(avgEIVs)){
  available_report_options <- available_report_options[names(available_report_options) != "EIVs (incl. Mean Hill-Ellenberg)"]
}
    
if(!is.null(habCor)){
  available_report_options <- available_report_options[names(available_report_options) != "Habitat Correspondence"]
}

reportOptions_options_named_df <- stack(lapply(available_report_options, names)) |>
  tibble::as_tibble() |>
  dplyr::select("Results Group" = ind, "Results Value" = values)

reportOptions_selected <- lapply(available_report_options, function(x) x[which(x %in% reportOptions)])
...
VC similarities for groups and years are not calculated when there are
...
```{r vcAssignmentTitle, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

reportOptions_vcAssignment <- RMAVIS:::reportOptions_options$`VC Assignment`

if(any(reportOptions_vcAssignment %in% reportOptions)){
...
```{r vcAssignmentResultsSiteCzekanowski, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if("vcAssignmentResultsSite_Czekanowski" %in% reportOptions){
  
  cat("## VC Assignment Czekanowski, by Site")
  
   if(!is.null(vcAssignmentSite_Czekanowski)){
     
    vcAssignmentSite_Czekanowski_prepped <- vcAssignmentSite_Czekanowski |>
      dplyr::mutate_if(is.numeric, round, digits = 2) |>
      dplyr::filter(VC.Code %in% vc_comms_subcomms_10)

    kableExtra::kbl(x = vcAssignmentSite_Czekanowski_prepped, format = "latex", booktabs = TRUE,
                    longtable = TRUE, linesep = "",
                    caption = "The top 10 VC communities fitted using the Czekanowski method, for the site by year."
                    ) |>
      kableExtra::kable_styling(latex_options = c("repeat_header", "HOLD_position"))
...
```{r vcAssignmentResultsGroupCzekanowski, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if("vcAssignmentResultsGroup_Czekanowski" %in% reportOptions){
  
  cat("## VC Assignment Czekanowski, by Group")
  
  if(!is.null(vcAssignmentGroup_Czekanowski)){
    
    vcAssignmentGroup_Czekanowski_prepped <- vcAssignmentGroup_Czekanowski |>
      dplyr::mutate_if(is.numeric, round, digits = 2) |>
      dplyr::filter(VC.Code %in% vc_comms_subcomms_10)

    kableExtra::kbl(x = vcAssignmentGroup_Czekanowski_prepped, format = "latex", booktabs = TRUE,
                    longtable = TRUE, linesep = "",
                    caption = "The top 10 VC communities fitted using the Czekanowski method, by group and year"
                    ) |>
      kableExtra::kable_styling(latex_options = c("repeat_header", "HOLD_position"))
...
```{r vcAssignmentResultsQuadrat, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if("vcAssignmentResultsQuadrat_Jaccard" %in% reportOptions){
  
  cat("## VC Assignment Jaccard, by Quadrat")
  ...
  if(!is.null(vcAssignmentPlot_Jaccard)){
    ...
    kableExtra::kbl(x = vcAssignmentPlot_Jaccard_prepped, format = "latex", booktabs = TRUE,
                    longtable = TRUE, linesep = "",
                    caption = "The top 10 VC communities fitted using the pseudo-quadrat method, by quadrat, group, and year") |>
      kableExtra::kable_styling(latex_options = c("repeat_header", "HOLD_position"))
...
if("habitatCorrespondenceSite" %in% reportOptions & !is.null(habCor)){
...
if("habitatCorrespondenceSite" %in% reportOptions & !is.null(habCor)){
  ...
  kableExtra::kbl(x = habCor_prepped, format = "latex", booktabs = TRUE,
                  longtable = TRUE, linesep = "",
                  caption = "The habitats associated with the top-N fitting VC communities from alternative habitat classifications.") |>
...
reportOptions_floristicTables <- RMAVIS:::reportOptions_options$`Floristic Tables`
...
reportOptions_eiv <- RMAVIS:::reportOptions_options$`EIVs (incl. Mean Hill-Ellenberg)`

if(any(reportOptions_eiv %in% reportOptions) & !is.null(weightedMeanHEValuesSite)){
...
if("weightedMeanHEValuesSite" %in% reportOptions & !is.null(weightedMeanHEValuesSite)){
...
if("unweightedMeanHEValuesSite" %in% reportOptions & !is.null(unweightedMeanHEValuesSite)){
...
if("weightedMeanHEValuesGroup" %in% reportOptions & !is.null(weightedMeanHEValuesGroup)){
...
if("unweightedMeanHEValuesGroup" %in% reportOptions & !is.null(unweightedMeanHEValuesGroup)){
...
if("weightedMeanHEValuesQuadrat" %in% reportOptions & !is.null(weightedMeanHEValuesQuadrat)){
...
if("unweightedMeanHEValuesQuadrat" %in% reportOptions & !is.null(unweightedMeanHEValuesQuadrat)){
...
reportOptions_diversity <- RMAVIS:::reportOptions_options$Diversity
...
```{r diversityYear, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if("diversity_year" %in% reportOptions){
  
  cat("## Diversity Measures - by Year")
  
  kableExtra::kbl(x = diversityYear, format = "latex", booktabs = TRUE,
                  longtable = TRUE, linesep = "",
                  caption = "Diversity Measures - by Year") |>
    kableExtra::kable_styling(latex_options = c("repeat_header", "HOLD_position"))
...
```{r diversityGroup, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if("diversity_group" %in% reportOptions){
  
  cat("## Diversity Measures - by Group")
  
  kableExtra::kbl(x = diversityGroup, format = "latex", booktabs = TRUE,
                  longtable = TRUE, linesep = "",
                  caption = "Diversity Measures - by Group") |>
    kableExtra::kable_styling(latex_options = c("repeat_header", "HOLD_position"))
...
```{r diversityQuadrat, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if("diversity_quadrat" %in% reportOptions){
  
  cat("## Diversity Measures - by Quadrat")
  
  kableExtra::kbl(x = diversityQuadrat, format = "latex", booktabs = TRUE,
                  longtable = TRUE, linesep = "",
                  caption = "Diversity Measures - by Quadrat") |>
    kableExtra::kable_styling(latex_options = c("repeat_header", "HOLD_position"))
...
reportOptions_mva <- RMAVIS:::reportOptions_options$MVA
  ...
  mvaNationalRefPlot <- ggplot2::ggplot() +
    ...
    ggplot2::geom_point(data = mvaNationalRef_quadrats_survey,
                        mapping = ggplot2::aes(Year = Year,
                                               ...
                                               x = DCA1,
                                               y = DCA2)) +
    ggplot2::geom_point(data = mvaNationalRef_centroids |> dplyr::filter(VC.Code %in% vc_comms_10),
                        mapping = ggplot2::aes(color = VC.Code,
                                               x = DCA1,
                                               y = DCA2)) +
    {if(!is.null(mvaNationalRef_CCA_arrowData))ggplot2::geom_segment(data = mvaNationalRef_CCA_arrowData,
                                                                     color = 'black',
                                                                     arrow = grid::arrow(),
    ...
    {if(!is.null(mvaNationalRef_CCA_arrowData))ggplot2::geom_text(data = mvaNationalRef_CCA_arrowData,
                                                                  color = 'black',
  ...
  mvaLocalRefRestrictedPlot <- ggplot2::ggplot() +
    ...
    ggplot2::geom_point(data = mvaLocalRefRestricted_quadrats_survey,
                        mapping = ggplot2::aes(Year = Year,
                                               ...
                                               x = DCA1,
                                               y = DCA2)) +
    ggplot2::geom_point(data = mvaLocalRefRestricted_centroids |> dplyr::filter(VC.Code %in% vc_comms_10),
                        mapping = ggplot2::aes(color = VC.Code,
                                               x = DCA1,
                                               y = DCA2)) +
    {if(!is.null(mvaLocalRefRestricted_CCA_arrowData))ggplot2::geom_segment(data = mvaLocalRefRestricted_CCA_arrowData,
                                                                            color = 'black',
                                                                            arrow = grid::arrow(),
                                                                            mapping = ggplot2::aes(x = 0,
                                                                                                   y = 0,
                                                                                                   ...
                                                                                                   yend = CCA2,
                                                                                                   label = `Hill-Ellenberg`))} +
    {if(!is.null(mvaLocalRefRestricted_CCA_arrowData))ggplot2::geom_text(data = mvaLocalRefRestricted_CCA_arrowData,
                                                                         color = 'black',
                                                                         # position = ggplot2::position_dodge(width = 0.9),
                                                                         size = 5,
                                                                         mapping = ggplot2::aes(x = CCA1 * 1.075,
  ...
  mvaLocalRefUnrestrictedPlot <- ggplot2::ggplot() +
    ...
    ggplot2::geom_point(data = mvaLocalRefUnrestricted_quadrats_survey,
                        mapping = ggplot2::aes(Year = Year,
                                               ...
                                               x = DCA1,
                                               y = DCA2)) +
    ggplot2::geom_point(data = mvaLocalRefUnrestricted_centroids |> dplyr::filter(VC.Code %in% vc_comms_10),
                        mapping = ggplot2::aes(color = VC.Code,
                                               x = DCA1,
                                               y = DCA2)) +
    {if(!is.null(mvaLocalRefUnrestricted_CCA_arrowData))ggplot2::geom_segment(data = mvaLocalRefUnrestricted_CCA_arrowData,
                                                                              color = 'black',
                                                                              arrow = grid::arrow(),
                                                                              mapping = ggplot2::aes(x = 0,
                                                                                                     y = 0,
                                                                                                     ...
                                                                                                     yend = CCA2,
                                                                                                     label = `Hill-Ellenberg`))} +
    {if(!is.null(mvaLocalRefUnrestricted_CCA_arrowData))ggplot2::geom_text(data = mvaLocalRefUnrestricted_CCA_arrowData,
                                                                           color = 'black',
                                                                           # position = ggplot2::position_dodge(width = 0.9),
                                                                           size = 5,
                                                                           mapping = ggplot2::aes(x = CCA1 * 1.075,

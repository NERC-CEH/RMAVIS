...
classifications is a well established practice in ecology and
conservation [@maciejewski2020]. The results of this assignment process
are used in various ways, including: informing ecological restoration
objectives [@gann2019] and assisting in the phase 2 habitat survey (or
NVC survey) [@rodwell2006national] process, along with establishing an
ecological baseline and identifying important ecological features such
as protected habitats [@cieem2022].

The development of computational methods and programs for the assignment
...
This R Shiny implementation of MAVIS is the newest program in this
...
Three methods for entering data into MAVIS are provided: manual, upload,
...
Four example datasets are available in MAVIS, which provide a means to
...
containing a total of 115 quadrats. The Domin scale of abundance system
was used, with these values being converted into direct percentage
estimates for use in MAVIS.
...
[@wallace2018]. No cover values are currently provided in this example
dataset.

## Survey Data Validation

At present MAVIS is designed to analyse vegetation survey data in the
...
Until these conditions are met MAVIS will not be able to proceed with as
the "Run Analysis" and "Generate Report" buttons will be disabled.

## NVC Assignment

In MAVIS NVC community similarities are assigned to survey data for the
site and each group of plots by year using the Czekanowski index
[@bloom1981; @hill1989], which quantifies the similarity 
between 1) a floristic table composed from survey data plots and 
2) the NVC floristic tables. 
...
The Multivariate Analysis (MVA) section of MAVIS aims to allow the user
...
communities, with the survey quadrats added in passively in MAVIS using
...
the site being analysed in MAVIS.
...
At present the **Download** section of MAVIS allows the user to
download:
